...
public class ContentTypeItemAnalyzer extends ClasspathAware {
    ...
    private final PackageHandler packageGenerator;
    ...
    private ClassReference getReturnType(Item item) {
        ...
        if (StringUtils.isBlank(namespace)) {
            result = getNoneHippoBeanReturnType(type);
        } else {
            result = getHippoBeanReturnType(item, type, namespace);
        }
        return result;
    }

    private ClassReference getHippoBeanReturnType(Item item, String type, String namespace) {
        ClassReference result;
        if (NodeType.HIPPO_MIRROR.equals(item.getType()) || NodeType.HIPPO_RESOURCE.equals(item.getType())) {
        ...
        } else if (getBeansOnClassPath().containsKey(type)) {
            result = new ClassReference(getBeansOnClassPath().get(type).getFullyQualifiedName());
        } else if (getBeansInProject().containsKey(type)) {
            result = new ClassReference(getBeansInProject().get(type).getFullyQualifiedName());
            ...
            result = new ClassReference(StringUtils.isBlank(packageName) ? className : packageName + "." + className);
        } else {
            result = new ClassReference(HippoBean.class);
        }
        return result;
    ...
    private ClassReference getNoneHippoBeanReturnType(String type) {
        ClassReference result;
    ...
    protected PackageHandler getPackageGenerator() {
